...
import { Directory, FileStructure, USER_CONNECTION_STATUS, User } from "./types/user"
...
io.on("connection", (socket: Socket) => {
	...
	// Handle file actions
	// SYNC_FILE_STRUCTURE
	socket.on(SocketEvent.SYNC_FILE_STRUCTURE,
		({ fileStructure, openFiles, activeFile, socketId }: {
			fileStructure: FileStructure; openFiles: File[]; activeFile: File; socketId: string;
		}) => {
	...
	socket.on(SocketEvent.DIRECTORY_CREATED,
	...
	socket.on(SocketEvent.DIRECTORY_UPDATED, ({ dirId, children }: { dirId: string; children: Directory[] }) => {
		const roomId = getRoomId(socket.id)
		if (!roomId) return
	...
	socket.on(SocketEvent.FILE_CREATED, ({ parentDirId, newFile }: { parentDirId: string; newFile: File }) => {
		const roomId = getRoomId(socket.id)
		if (!roomId) return
		socket.broadcast
			.to(roomId)
	...
	// Handle chat actions
	socket.on(SocketEvent.SEND_MESSAGE, ({ message }: { message: string }) => {
		const roomId = getRoomId(socket.id)
		if (!roomId) return
		socket.broadcast
			.to(roomId)
	...
	socket.on(SocketEvent.TYPING_START, ({ cursorPosition }: { cursorPosition: number }) => {
		userSocketMap = userSocketMap.map((user) => {
	...
	socket.on(SocketEvent.SYNC_DRAWING, ({ drawingData, socketId }: {
		drawingData: string; socketId: string;
	}) => {
		socket.broadcast
			.to(socketId)
